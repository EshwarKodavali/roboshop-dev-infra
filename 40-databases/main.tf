...
resource "terraform_data" "mongodb" {
  triggers_replace = [
    aws_instance.mongodb.id # This trigger ensures the script runs every time Terraform applies
  ]
...
resource "terraform_data" "redis" {
  triggers_replace = [
    aws_instance.redis.id # This trigger ensures the script runs every time Terraform applies
  ]
...
resource "aws_route53_record" "mongodb" {
  zone_id = var.zone_id
  name    = "mongodb-${var.env}.${var.domain_name}" # mongodb-dev.eshwar.fun
  type    = "A"
  ttl     = 1
